...
public class HashTable<E>
{
    ...
    public String toString (int limit){
        ...
        for (int i=0; i < array.length && ct < limit; i++){
            if (array[i]!=null && array[i].isActive) {
                if (array[i].element instanceof WordInfo) {
                    sb.append( "     " + i + ": " + array[i].element.toString() + " "+ (((WordInfo) array[i].element).getTimesUsed() + 1) + "\n");
                } else {
     ...
     * 
     */
    private int myhash( E x, int attempt)
    {
